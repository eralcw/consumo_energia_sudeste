...
def overall_col_col_average(coluna_1, coluna_2):
    try:
        df = pd.read_csv('./data/raw/raw_0.csv')
        show = (
            df.groupby([coluna_1, coluna_2])['Consumo']
            .agg(
                Quantidade='count',
                Media='mean',
                Mediana='median',
        ...
        print(show)
        path = f'./data/processed/overall_{coluna_1.lower()}_{coluna_2.lower()}_average.csv'
        if not os.path.exists(path):
            show.to_csv(path, index=False)
    except Exception as e:
        print(f'Erro ao calcular overall por Regiao e Classe: {e}')
...
if __name__=='__main__':
    overall_col_col_average('Sistema', 'Classe')